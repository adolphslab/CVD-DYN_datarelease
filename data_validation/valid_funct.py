...
def check_for_errors(data, valid_data, keyword):
   ...
   (len(set(valid_data['CVDID']+valid_data['wave']) - set(data['CVDID']+data['wave'])) !=0) |
   (len(set(data['CVDID']+data['wave'])- set(valid_data['CVDID']+valid_data['wave']))  !=0)):
        warnings.warn("Warning: Check " + keyword + "!");


def attent_check(dat, w):
    """
    Count number of failed attention questions.
    INPUT
    ...
    OUTPUT
        out (pd.DataFrame) - 7 columns: 
                                * wave
                                * CVDID
                                * 5 possible failed attention questions (TRUE if failed, FALSE if passed, NaN if not included)
    """
    ...
    return out[['CVDID', 'wave', 'more_than_1_attQ_failed']]

def w_completed(data):
    w_completed = pd.DataFrame(data[['CVDID', 'wave', 'V5']])
...
def nounVerb_count(data):
    ...
    verbNoun_df = data[['CVDID', 'wave']]

    for text_col in text_cols:
        df  = data[['CVDID', 'wave', text_col]].dropna().reset_index(drop = True)
        ...
        verbNoun_df = verbNoun_df.merge(df, on = ['CVDID', 'wave'], how = 'left')
    ...
    return verbNoun_df[['CVDID', 'wave','free_text_resp_valid_stress', 'free_text_resp_valid_news']]

####################################### RESP STRING LENGTH #############################
def extract_long_string(long_string_df, data, name, input_vars):
    tmp_df = run_analysis(data, name, input_vars)
    long_string_df = long_string_df.merge(tmp_df, on = ['CVDID', 'wave'], how = 'left')
